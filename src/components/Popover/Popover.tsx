/** Source: https://github.com/elite174/solid-simple-popover */
...
const getContentElement = (
    ...
    if (!(element instanceof HTMLElement))
        throw new Error('content must be HTML element');

    if (elementSelector) {
        element = element.matches(elementSelector)
            ? element
            : element.querySelector(elementSelector);

        if (!(element instanceof HTMLElement))
            throw new Error(
                `Unable to find element with selector "${elementSelector}"`,
            );
    }
...
export const Popover: ParentComponent<PopoverProps> = (initialProps) => {
    ...
    createEffect(() => {
        ...
        createEffect(() =>
            trigger.setAttribute(dataAttributeName, String(open())),
        );
                ...
                createEffect(() => {
                    ...
                    createEffect(() => {
                        if (typeof props.targetPositionArea === 'string') {
                            ...
                            typeof props.targetPositionArea === 'object'
                        ) {
                            const targetPositionAreaObject =
                                props.targetPositionArea;

                            content.style.top =
                                untrack(() =>
                                    targetPositionAreaObject.top?.(anchorName),
                                ) ?? '';
                            content.style.left =
                                untrack(() =>
                                    targetPositionAreaObject.left?.(anchorName),
                                ) ?? '';
                            content.style.right =
                                untrack(() =>
                                    targetPositionAreaObject.right?.(
                                        anchorName,
                                    ),
                                ) ?? '';
                            content.style.bottom =
                                untrack(() =>
                                    targetPositionAreaObject.bottom?.(
                                        anchorName,
                                    ),
                                ) ?? '';
                    ...
                    createEffect(() => {
                        ...
                        const handleClickOutside = (e: MouseEvent) => {
                            ...
                            if (
                                eventPath.includes(trigger) ||
                                eventPath.includes(content)
                            )
                                return;
